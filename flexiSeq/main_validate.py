...
from datasets.variable_length_dataset import VariableLengthDataset
if __name__ == '__main__':
    # Define dataset paths.
    dataset_paths = {
        'Kohli_data': "../new_kohli_data",
        # 'Train': "../Train",
        'Test_divyansh': "../Test_divyansh",
        'Test_krrish': "../Test_krrish",
        'pujara_data': "../pujara_data",
    }
    ...
    results = cv.validate(folds=5)

    print("\nFinal Results:")
    print(f"Accuracy: {results[0]:.4f}, AUC: {results[1]:.4f}, F1: {results[2]:.4f}, F1 (weighted): {results[3]:.4f}")
    # for i, (acc, auc, f1) in enumerate(results):
    #     print(f"Fold {i+1}: Accuracy = {acc:.4f}, AUC = {auc:.4f}, F1 = {f1:.4f}")
